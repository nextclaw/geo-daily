(function () {
  ...
  function renderMath(container) {
    if (
      window.JanusSite &&
      typeof window.JanusSite.renderMathInContainer === "function"
    ) {
  ...
  async function renderMermaid(container) {
    ...
    try {
      ...
      await mermaidModule.run({ nodes: mermaidBlocks });
    } catch (err) {
      console.warn("[janus] mermaid render failed:", err);
    }
  }
  ...
  async function loadArticle(slug, linkEl) {
    ...
    contentPane.innerHTML =
      '<div class="explorer-loading"><span>⏳</span><p>Loading article...</p></div>';
    ...
    try {
      ...
      await renderMermaid(contentPane);
    } catch (error) {
      console.warn("[janus] explorer load failed:", error);
      contentPane.innerHTML =
        '<div class="explorer-error"><p>Failed to load article preview.</p></div>';
    }
  }
  ...
  function renderNode(node) {
    ...
    if (node.slug) {
      ...
      link.innerHTML =
        ...
        (node.date
          ? `<span class="file-date">${escapeHtml(node.date)}</span>`
          : "");
  ...
  if (!tree.children || tree.children.length === 0) {
    root.innerHTML =
      '<div class="explorer-empty"><p>No articles found.</p></div>';
    return;
  }
